...
public class AppletDisplay extends Applet{
	...
	inputlabel = new JLabel("Input:");
		...
		htmlpathbox = new JTextField("Input xml file to read from",25);
		htmlpathbox.setEnabled(false);
		
		htmlbrowsebutton = new JButton("Browse");
		...
		xmloutbrowsebutton = new JButton("Browse");
	...
	private void renderLayout(){
		...
		gba.shiftRight(1);
		gba.addComponent(exitbutton,2,1);
	...
	/*private void start(){
		...
		if(lookupradio.isSelected()){
			...
			back = new Backend(inputpathbox.getText(),dictionary);
		}else{
			statusbar.setText("Reading the XML File. Please Wait.....");
			back = new Backend(xmlpathbox.getText());
		}
	...
	private class GBAssistant{
		...
		private AppletDisplay panel;
		
		private int x=0, y=0;
		
		private int gw=1, gh=1;
		
		public GBAssistant(AppletDisplay panel, GridBagConstraints constraints){
			this.panel = panel;
